...
const INTERNAL_DIAGONAL = [
    2013265919n, 1n, 2n, 1006632961n, 3n, 4n, 1006632960n, 2013265918n,
    2013265917n, 2005401601n, 1509949441n, 1761607681n, 2013265906n,
    7864320n, 125829120n, 15n,
];
...
function sized(bytes, length, name) {
...
function integer(value, length) {
    if (value < 0n || value >= 1n << BigInt(length * 8))
        throw new RangeError(`integer does not fit ${length} bytes`);
    const out = new Uint8Array(length);
...
function externalLinearLayer(state) {
    ...
    for (let i = 0; i < 16; i++)
        sums[i & 3] = add(sums[i & 3], state[i]);
    for (let i = 0; i < 16; i++)
        state[i] = add(state[i], sums[i & 3]);
}
function externalRounds(state, constants) {
    for (const round of constants) {
        for (let i = 0; i < 16; i++)
            state[i] = sbox(add(state[i], round[i]));
        externalLinearLayer(state);
    }
}
export function poseidon2BabyBear16(input) {
    if (input.length !== 16)
        throw new RangeError("Poseidon2 state must contain 16 elements");
    ...
    for (const constant of INTERNAL) {
        ...
        for (const value of state)
            sum = add(sum, value);
        for (let i = 0; i < 16; i++)
            state[i] = add(sum, mul(state[i], INTERNAL_DIAGONAL[i]));
    }
    externalRounds(state, EXTERNAL_FINAL);
    return Uint32Array.from(state, (value) => Number(value));
}
export function digestElements(digest, name = "digest") {
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if (value >= BABY_BEAR_MODULUS)
            throw new RangeError(`${name} element ${i} is not canonical`);
        elements[i] = value;
    }
    return elements;
}
export function digestFromElements(elements) {
    if (elements.length !== 16)
        throw new RangeError("digest must contain 16 elements");
...
function byteElements(bytes) {
    const elements = new Array(Math.ceil(bytes.length / 2));
    for (let i = 0; i < elements.length; i++)
        elements[i] = (bytes[i * 2] << 8) | (bytes[i * 2 + 1] ?? 0);
    return elements;
}
function secretElements(secret, name) {
    if (secret.length !== 8)
        throw new RangeError(`${name} must contain 8 limbs`);
    const elements = new Array(8);
    for (let index = 0; index < elements.length; index++) {
        const input = secret[index];
        if ((typeof input === "number" && (!Number.isInteger(input) || input < 0 || input >= BABY_BEAR_MODULUS))
...
export function secretBytes(secret, name = "secret") {
    const elements = secretElements(secret, name);
    const bytes = new Uint8Array(32);
    const view = new DataView(bytes.buffer);
    for (let index = 0; index < elements.length; index++)
        view.setUint32(index * 4, elements[index], false);
    return bytes;
}
export function secretFromBytes(bytes, name = "secret") {
    sized(bytes, 32, name);
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    ...
    while (accepted < secret.length) {
        ...
        for (let offset = 0; offset < entropy.length && accepted < secret.length; offset += 4) {
            ...
            if (candidate < BABY_BEAR_MODULUS)
                secret[accepted++] = candidate;
        }
    }
    return secret;
}
export function p2bb512(tag, payloadByteLength, aux, payload) {
    if (!Number.isInteger(tag) || tag < 0 || tag > 255)
        throw new RangeError("tag must be one byte");
    ...
    if (!Number.isInteger(aux) || aux < 0 || aux >= BABY_BEAR_MODULUS)
        throw new RangeError("aux is not canonical");
...
export function k512(tag, payload) {
    if (!Number.isInteger(tag) || tag < 0 || tag > 255)
        throw new RangeError("tag must be one byte");
    return {
...
export function scope(input) {
    ...
    const payload = concat(integer(input.chainId, 8), sized(input.pool, 20, "pool"), integer(input.denomination, 32), integer(BigInt(depth), 1), integer(BigInt(version), 4), digestBytes(input.parameterId));
    return p2bb512(domains.SCOPE, payload.length, 0, byteElements(payload));
}
export function commitment(poolScope, nullifierSecret, trapdoor) {
...
export function nullifierHash(poolScope, nullifierSecret) {
    return p2bb512(domains.NULLIFIER, 96, 0, [
        ...digestElements(poolScope, "scope"),
        ...secretElements(nullifierSecret, "nullifierSecret"),
    ]);
}
export function emptyLeaf(poolScope) {
    return p2bb512(domains.EMPTY_LEAF, 64, 0, digestElements(poolScope, "scope"));
}
export function merkleNode(level, left, right) {
    if (!Number.isInteger(level) || level < 0 || level >= TREE_DEPTH)
        throw new RangeError("invalid Merkle level");
    return p2bb512(domains.APP_MERKLE_NODE, 128, level, [...digestElements(left, "left"), ...digestElements(right, "right")]);
}
export function payoutDigest(recipient, relayer, fee) {
...
export function publicValues(statement) {
...
export function encodeCompactProof(proof) {
    if (proof.publicValues.length !== PUBLIC_VALUES_COUNT)
        throw new RangeError("public value count must be 64");
    if (proof.starkPayload.length > MAX_STARK_PAYLOAD_BYTES)
        throw new RangeError("STARK payload is too large");
    const parameter = digestBytes(proof.parameterId);
    if (parameter.every((byte) => byte === 0))
        throw new Error("parameter ID cannot be zero");
    ...
    for (let i = 0; i < PUBLIC_VALUES_COUNT; i++) {
        ...
        if (value >= BABY_BEAR_MODULUS)
            throw new RangeError(`field element ${value} is not canonical`);
        fieldView.setUint32(i * 4, value, false);
    }
    return concat(PROOF_MAGIC, integer(3n, 2), parameter, integer(BigInt(PUBLIC_VALUES_COUNT), 2), fields, integer(BigInt(proof.starkPayload.length), 4), proof.starkPayload, PROOF_END);
}
export function decodeCompactProof(bytes) {
    const fixedPrefix = 8 + 2 + 64 + 2 + PUBLIC_VALUES_COUNT * 4 + 4;
    if (bytes.length < fixedPrefix + 4)
        throw new Error("proof is truncated");
    let cursor = 0;
    const take = (length) => {
    ...
    const readU32 = () => new DataView(take(4).buffer).getUint32(0, false);
    if (!equalBytes(take(8), PROOF_MAGIC))
        throw new Error("invalid proof magic");
    if (readU16() !== 3)
        throw new Error("unsupported proof codec version");
    const parameterId = digestFromBytes(take(64));
    if (digestBytes(parameterId).every((byte) => byte === 0))
        throw new Error("parameter ID cannot be zero");
    if (readU16() !== PUBLIC_VALUES_COUNT)
        throw new Error("public value count must be 64");
    ...
    for (let i = 0; i < PUBLIC_VALUES_COUNT; i++) {
        ...
        if (value >= BABY_BEAR_MODULUS)
            throw new Error(`field element ${value} is not canonical`);
        publicValues[i] = value;
    }
    const payloadLength = readU32();
    if (payloadLength > MAX_STARK_PAYLOAD_BYTES)
        throw new Error("STARK payload is too large");
    const starkPayload = take(payloadLength);
    if (!equalBytes(take(4), PROOF_END))
        throw new Error("invalid proof end marker");
    if (cursor !== bytes.length)
        throw new Error("proof has trailing bytes");
    return { parameterId, publicValues, starkPayload };
}
export function newNote(chainId, pool, parameterId) {
...
export function encodeNote(note) {
    const body = concat(textEncoder.encode("PQTN"), integer(3n, 2), integer(note.chainId, 8), sized(note.pool, 20, "pool"), digestBytes(note.parameterId), secretBytes(note.nullifierSecret, "nullifierSecret"), secretBytes(note.trapdoor, "trapdoor"));
...
export function parseNote(encoded) {
    ...
    if (!/^[A-Za-z0-9_-]+$/.test(payload))
        throw new Error("invalid note encoding");
    const bytes = new Uint8Array(Buffer.from(payload, "base64url"));
    if (Buffer.from(bytes).toString("base64url") !== payload)
        throw new Error("noncanonical note encoding");
    if (bytes.length !== NOTE_LENGTH)
        throw new Error("invalid note length");
    if (new TextDecoder().decode(bytes.slice(0, 4)) !== "PQTN")
        throw new Error("invalid note magic");
    ...
    if (!equalBytes(expected, bytes.slice(162)))
        throw new Error("invalid note checksum");
    let chainId = 0n;
    for (const byte of bytes.slice(6, 14))
        chainId = (chainId << 8n) | BigInt(byte);
    return {
        chainId,
...
export class NoteSecretTracker {
    nullifierSecrets = new Set();
    register(note) {
        const key = Buffer.from(secretBytes(note.nullifierSecret, "nullifierSecret")).toString("hex");
        if (this.nullifierSecrets.has(key))
            throw new Error("nullifier secret was already used locally");
...
export class MerkleTree {
    ...
    root;
    constructor(poolScope) {
        this.zeros = [emptyLeaf(poolScope)];
        for (let level = 0; level < TREE_DEPTH; level++)
            this.zeros.push(merkleNode(level, this.zeros[level], this.zeros[level]));
    ...
    insert(leaf) {
        ...
        for (let level = 0; level < TREE_DEPTH; level++) {
            if ((index & 1) === 0) {
                ...
                current = merkleNode(level, current, this.zeros[level]);
            }
            else
                current = merkleNode(level, this.filledSubtrees[level], current);
            index >>= 1;
        }
    ...
    path(leafIndex) {
        if (!Number.isInteger(leafIndex) || leafIndex < 0 || leafIndex >= this.leaves.length)
            throw new RangeError("unknown leaf");
        const siblings = [];
        ...
        for (let level = 0; level < TREE_DEPTH; level++) {
            ...
            if ((nodes.length & 1) === 1)
                nodes.push(this.zeros[level]);
            const parents = [];
            for (let i = 0; i < nodes.length; i += 2)
                parents.push(merkleNode(level, nodes[i], nodes[i + 1]));
            nodes = parents;
            index >>= 1;
        }
...
export function rootFromPath(leaf, path) {
    if (path.siblings.length !== TREE_DEPTH)
        throw new RangeError("path must have 20 siblings");
    let current = leaf;
    for (let level = 0; level < TREE_DEPTH; level++)
        current = (path.leafIndex & (1 << level)) === 0
...
export function encodeDepositCalldata(noteCommitment) {
    return concat(selector("deposit((bytes32,bytes32))"), sized(noteCommitment.left, 32, "commitment.left"), sized(noteCommitment.right, 32, "commitment.right"));
}
export function hex(bytes) {
    return `0x${Buffer.from(bytes).toString("hex")}`;
}
export function bytesFromHex(value, expectedLength) {
    if (!/^0x[0-9a-fA-F]*$/.test(value) || value.length % 2 !== 0)
        throw new TypeError("invalid hex");
    const bytes = Uint8Array.from(Buffer.from(value.slice(2), "hex"));
    if (expectedLength !== undefined)
        sized(bytes, expectedLength, "hex value");
    return bytes;
}
