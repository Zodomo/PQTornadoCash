...
The frozen v0.3 direct deposit measurement is exactly 13,991,021 gas. The isolated Foundry harness retained 256 enqueue gas-left deltas (121,922–143,822), but these are `ISOLATED_ENQUEUE_DIAGNOSTIC` values, not a complete user-deposit transaction: intrinsic gas, calldata gas, the EIP-7623 floor, and a receipt are absent. Complete queue user-deposit gas, finalizer gas, proof bytes, proving/verification time, and RSS remain `NOT_EVALUATED`. The main generator validates and preserves the retained artifact on rerun. Analytic Poisson fill latency is a model, not a timing measurement, and no amortized break-even is produced from missing inputs.
