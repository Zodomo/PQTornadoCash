...
Measure the published sponge as a non-Poseidon comparator; never wrap it as a compressor. The layout exercise absorbs generic payload after role/version/shape/level using published padding/domain rules, but note/nullifier payloads omit scope and frozen semantic secret/trapdoor widths and are not application-complete.
...
- 5× node-permutation reduction: FAIL — H0 uses 11 node permutations; H7 uses 3; 3.667× reduction.
...
- No structural attack: NOT PASSED — exact application-framing structural analysis is missing.
