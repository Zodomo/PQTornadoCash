...
Execute primitive and layout benchmarks only. The 20 generic payload lanes followed by role/version/shape/level exactly fill 24 lanes, but note/nullifier payloads omit scope and frozen semantic secret/trapdoor widths and are not application-complete.
...
- 5× node-permutation reduction: PASS — H0 uses 11 node permutations; H3 uses 1; 11.000× reduction.
...
- Generic security of at least 100 bits: PASS — ideal quantum collision ceiling 103.023 bits and hidden-part Grover ceiling 216.348 bits; limiting generic ceiling 103.023 bits. This is a necessary width heuristic only, not qualification.
- No structural attack: FAIL — the exact 2026 width-24, d=10 compression round-skip result applies.
