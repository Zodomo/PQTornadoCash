...
pub const QUERY_COUNT: usize = if cfg!(feature = "q48") { 48 } else { 32 };
pub const QUERY_SPLITS: &[usize] = if cfg!(feature = "q48") { &[22, 24, 26] } else { &[12, 14, 16] };
...
impl SecurityProfile {
    ...
    pub const fn fri(self) -> (usize, usize, usize, usize, usize) {
        match self {
            Self::Dev => (3, 0, 2, 0, 0),
            Self::Ci => (3, 0, 16, 4, 4),
            Self::SepoliaV03 => (4, 0, QUERY_COUNT, 16, 16),
        }
    }
...
pub fn config_from_os_entropy(
    ...
    assert_eq!(profile, SecurityProfile::SepoliaV03, "T3-02 implements the compiled H0 query schedule");
