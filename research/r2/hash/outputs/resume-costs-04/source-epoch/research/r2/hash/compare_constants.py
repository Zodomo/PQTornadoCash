...
P=2013265921
ALLOWED={'PATH','HOME','USER','TMPDIR','CARGO_HOME','RUSTUP_HOME','RUSTUP_TOOLCHAIN','CARGO_TARGET_DIR','CARGO_BUILD_JOBS','RUSTFLAGS','SDKROOT','MACOSX_DEPLOYMENT_TARGET','CC','CXX','AR','LIBRARY_PATH','DYLD_LIBRARY_PATH','OMP_NUM_THREADS','RAYON_NUM_THREADS'}
def clean_env():return {k:v for k,v in os.environ.items() if k in ALLOWED}
def main():
    p=argparse.ArgumentParser();p.add_argument('--plonky3',type=Path,required=True);p.add_argument('--binary',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();out=a.output.resolve()
    if not out.is_relative_to(ROOT) or not any(part.startswith('resume-')for part in out.relative_to(ROOT).parts):p.error('output must be new and within a resume-* directory under research/r2/hash')
    ...
    for i,f in enumerate(files):
        if f.suffix in ('.py','.rs','.json'):
            retained=out/'source-epoch'/f'{i}-{f.name}';retained.parent.mkdir(parents=True,exist_ok=True);retained.write_bytes(f.read_bytes())
