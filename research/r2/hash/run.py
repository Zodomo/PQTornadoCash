...
"""
import argparse,collections,hashlib,json,os,random,shutil,subprocess,sys,tempfile,time,urllib.request,urllib.parse
...
class RPC:
    ...
    def deploy(self,name,args=b''):
        artifact=json.loads((ROOT/'solidity/out'/('Deposit.sol'if name=='Deposit'else'Roles.sol')/(name+'.json')).read_text());code=bytes.fromhex(artifact['bytecode']['object'].removeprefix('0x'))
        ...
        receipt=self.transact(initcode)
        if int(receipt['status'],16)==1:
            runtime=self.call('eth_getCode',[receipt['contractAddress'],'latest']);metadata['actual_runtime_bytes']=len(runtime[2:])//2;metadata['runtime_code_deposit_gas']=200*metadata['actual_runtime_bytes']
            dump(OUT/'initcode'/(self.label+'-'+receipt['transactionHash'][2:]+'.json'),{'initcode':'0x'+initcode.hex(),'runtime':runtime,'receipt':receipt})
...
def trace(rpc,receipt,label):
    result=rpc.call('debug_traceTransaction',[receipt['transactionHash'],{'disableStorage':True,'disableMemory':True,'disableStack':True}]);dump(OUT/'traces'/(label+'.json'),result)
    ...
    return {'receipt_gas':int(receipt['gasUsed'],16),'opcode_counts':dict(counts),'opcode_reported_costs':dict(costs),'categories':attribution,'classification':'overlapping opcode attribution, not additive causal deltas; CALL cost includes callee budget; constant access overlaps memory/dispatch; source strategies alone do not establish causal gas savings'}

def deploy_tiers(rpc):
    ...
    for config,tiers in addresses.items():
        for tier,address in tiers.items():
            ...
            try:
                ...
                if int(receipt['status'],16)!=1:records.append(record);continue
                ...
                if do_trace:
                    record['constructor_attribution']=trace(rpc,receipt,rpc.label+'-'+tier+'-constructor')
                    record['deposit_attribution']=trace(rpc,dep,rpc.label+'-'+tier+'-deposit')
            except RuntimeError as e:record['error']=str(e)
            records.append(record);dump(OUT/(rpc.label+'-deposit.json'),records)
    return records
...
def main():
    ...
    if a.build:
        command(['cargo','build','--release','--manifest-path',ROOT/'rust/Cargo.toml'],'cargo-build')
        with tempfile.TemporaryDirectory(prefix='pqtc-r2-hash-forge-',dir='/tmp')as directory:
            ...
            (work/'foundry.toml').write_text(config);command(['forge','build','--root',work],'forge-build',cwd=work);shutil.copytree(work/'out',ROOT/'solidity/out',dirs_exist_ok=True)
    binary=ROOT/'target/release/pqtc-r2-hash'
    if a.plonky3:command([sys.executable,ROOT/'compare_constants.py','--plonky3',a.plonky3,'--binary',ROOT/'target/release/constants'],'constants-comparison')
    result={'specification':'complete R2-H5-complete-v1; exact frozen H0 control','correctness':'pending','privacy':'synthetic-unfunded only; no hiding claim','security':'security-not-qualified','performance':'pending','stage':'executing-isolated-measurements','promotion':'not-promoted','argv':sys.argv,'environment':environment,'source_corpus_sha256':hashlib.sha256((REPO/'research/r2/corpus/semantic-cases.json').read_bytes()).hexdigest()};dump(OUT/'results.json',result)
    sources=[f for f in ROOT.rglob('*')if f.is_file()and f.suffix in ('.rs','.sol','.ts','.py','.toml','.json')and not any(part in ('target','out','outputs','cache')for part in f.relative_to(ROOT).parts)]
    ...
    primary=RPC(a.rpc_url,'capped-local-top-level',a.gas_limit);primary_addresses,deployment=deploy_tiers(primary)
    diagnostic=RPC(a.diagnostic_rpc_url,'diagnostic-unpromotable',a.diagnostic_gas_limit)if a.diagnostic_rpc_url else primary
    addresses,diag_deploy=deploy_tiers(diagnostic)if diagnostic is not primary else(primary_addresses,deployment)
    ...
    try:
        ...
        with (OUT/'vectors.jsonl').open('w')as vectors,(OUT/'parity.jsonl').open('w')as parity:
            import itertools
            source=itertools.chain(full_vectors(a.vectors),corpus_vectors(native));batch=[]
            def flush(batch):
                nonlocal checked
        ...
        result['parity']={'checked_shared_full_roles':checked,'coverage':dict(coverage),'chunk_size':a.chunk};result['misuse_cases']=misuse(native,diagnostic,addresses)
        benchmarks=[]
        for config in CONFIGS:
            d=16 if config==CONFIGS[0]else 12
            ...
            native_bench=command([binary,a.native_repetitions],'native-'+config,'\n'.join(json.dumps(r)for r in samples)+'\n');dump(OUT/('native-'+config+'.json'),{'requests':samples,'measurements':[json.loads(x)for x in native_bench.splitlines()],'binary_bytes':binary.stat().st_size,'measurement_class':'native-latency-no-proof-performance-claim'})
            for tier,address in addresses[config].items():
                for r in samples:
                ...
                gas=diagnostic.call('eth_estimateGas',[{'to':address,'from':diagnostic.account,'data':'0x'+data.hex(),'gas':hex(diagnostic.gas)}]);benchmarks.append({'config':config,'tier':tier,'operation':'twenty-level-hashing','gas':int(gas,16),'measurement_class':'eth_estimateGas-compute-only'} )
        ...
        native.close();dump(OUT/'results.json',result);dump(OUT/'artifact-ledger.json',{str(f.relative_to(ROOT)):{'bytes':f.stat().st_size,'sha256':hashlib.sha256(f.read_bytes()).hexdigest()}for f in OUT.rglob('*')if f.is_file()and f.name!='artifact-ledger.json'})
