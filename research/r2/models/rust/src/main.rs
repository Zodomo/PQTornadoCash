...
fn main() -> Result<()> {
    ...
    ensure!(cli.cap_height <= manifest.fri_log_blowup, "cap exceeds final committed tree height");
    ...
    for repetition in 0..cli.repetitions {
        let config = config_from_os_entropy(&manifest, parameter, &pvs, cli.cap_height);
        ...
        fs::write(cli.out.join(format!("proof-{repetition}.bin")), &canonical)?;
    ...
    let output = json!({
        "schemaVersion": 1,
        "capHeight": cli.cap_height,
        "sourceFork": "research/candidates/C10-fri/src/main.rs",
        "candidateId": "C10-fri",
...
fn config_from_os_entropy(m: &Manifest, parameter_id: Digest512, pvs: &[Val], cap_height: usize) -> Config {
    let mut os_rng = rand::rng();
    let mmcs_rng = StdRng::from_rng(&mut os_rng);
    let pcs_rng = StdRng::from_rng(&mut os_rng);
    let val_mmcs = ValMmcs::new(ProofLeafHasher, ProofNodeCompressor, cap_height, mmcs_rng);
...
fn peak_rss_bytes() -> Option<u64> {
    ...
    if unsafe { libc::getrusage(libc::RUSAGE_SELF, &mut usage) } == 0 { Some(usage.ru_maxrss as u64) } else { None }
}

#[cfg(not(target_os = "macos"))]
fn peak_rss_bytes() -> Option<u64> {
    let mut usage = unsafe { std::mem::zeroed::<libc::rusage>() };
    if unsafe { libc::getrusage(libc::RUSAGE_SELF, &mut usage) } == 0 { Some((usage.ru_maxrss as u64) * 1024) } else { None }
}
